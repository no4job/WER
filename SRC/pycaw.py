...
class AudioUtilities(object):
    ...
    @staticmethod
    def GetAllDevices_(flow = EDataFlow.eAll.value,state = DEVICE_STATE.MASK_ALL.value):
    # def GetAllDevices():
        devices = []
        deviceEnumerator = comtypes.CoCreateInstance(
            CLSID_MMDeviceEnumerator,
    ...
    @staticmethod
    # def GetDeviceVolume(deviceEnumerator,device = None,device_ = None):
    def GetDeviceVolume(deviceEnumerator,device):
        # if not device == None and device_ == None:
        #     device_ = deviceEnumerator.GetDevice(device.id)
        # elif device == None and not device_ == None:
        #     pass
        # else:
        #     return None
        device_ = deviceEnumerator.GetDevice(device.id)
        interface = device_.Activate(IAudioEndpointVolume._iid_, comtypes.CLSCTX_ALL, None)
        volume_control_ptr = cast(interface, POINTER(IAudioEndpointVolume))
        device_volume = {}
        # if not device == None:
        #     device_volume["GUID"] =  device.id
        # else:
        #     device_volume["GUID"] =  device_.GetId()
        device_volume["GUID"] =  device.id
        device_volume["Name"] =  re.sub('[^:]*:','',str(device)).strip()
        device_volume["MasterVolumeLevel"] =  volume_control_ptr.GetMasterVolumeLevel()
        device_volume["VolumeLevelScalar"] =  volume_control_ptr.GetMasterVolumeLevelScalar()
        device_volume["VolumeRange"] =  volume_control_ptr.GetVolumeRange()
        return device_volume

    @staticmethod
    def GetAllDeviceVolumeTh(all_device_volume):
        all_device_volume[0] = AudioUtilities.GetAllDeviceVolume()
        return

    @staticmethod
    def GetAllDeviceVolume():
        all_in_devices = AudioUtilities.GetAllDevices_(EDataFlow.eCapture.value,DEVICE_STATE.ACTIVE.value)
        all_out_devices = AudioUtilities.GetAllDevices_(EDataFlow.eRender.value,DEVICE_STATE.ACTIVE.value)
        deviceEnumerator = comtypes.CoCreateInstance(CLSID_MMDeviceEnumerator, IMMDeviceEnumerator,
                                                     comtypes.CLSCTX_INPROC_SERVER)
        AllDeviceVolume = {}

        device_volume_list = []
        for device in all_in_devices:
            device_volume = AudioUtilities.GetDeviceVolume(deviceEnumerator,device)
            device_volume_list.append(device_volume)
        AllDeviceVolume['in'] = device_volume_list

        # deviceEnumerator = comtypes.CoCreateInstance(CLSID_MMDeviceEnumerator, IMMDeviceEnumerator,
        #                                              comtypes.CLSCTX_INPROC_SERVER)
        device_volume_list = []
        for device in all_out_devices:
            device_volume = AudioUtilities.GetDeviceVolume(deviceEnumerator,device)
            device_volume_list.append(device_volume)
        AllDeviceVolume['out'] = device_volume_list

        # deviceEnumerator = comtypes.CoCreateInstance(CLSID_MMDeviceEnumerator, IMMDeviceEnumerator,
        #                                              comtypes.CLSCTX_INPROC_SERVER)
        default_in_device = deviceEnumerator.GetDefaultAudioEndpoint(EDataFlow.eCapture.value, DEVICE_STATE.ACTIVE.value)
        # device_volume = AudioUtilities.GetDeviceVolume(deviceEnumerator,device_= default_in_device)
        device_volume = AudioUtilities.GetDeviceVolume(deviceEnumerator,device = AudioUtilities.CreateDevice(default_in_device))
        AllDeviceVolume['default_in'] = device_volume

        # deviceEnumerator = comtypes.CoCreateInstance(CLSID_MMDeviceEnumerator, IMMDeviceEnumerator,
        #                                              comtypes.CLSCTX_INPROC_SERVER)
        default_out_device = deviceEnumerator.GetDefaultAudioEndpoint(EDataFlow.eRender.value, DEVICE_STATE.ACTIVE.value)
        # device_volume = AudioUtilities.GetDeviceVolume(deviceEnumerator,device_= default_out_device)
        device_volume = AudioUtilities.GetDeviceVolume(deviceEnumerator,device= AudioUtilities.CreateDevice(default_out_device))
        AllDeviceVolume['default_out'] = device_volume

        return AllDeviceVolume

if __name__ == '__main__':
    print("Start AudioUtilities.GetAllDeviceVolume()")
    PYCAW_IPC_FILE = '..\\WRK\\IPC\\AllDeviceVolume.pickle'
    AllDeviceVolume = AudioUtilities.GetAllDeviceVolume()
    with open(PYCAW_IPC_FILE, 'wb') as f:
        pickle.dump(AllDeviceVolume, f)
